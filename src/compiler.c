...
static void method() {
  ...
  if (parser.previous.length == INITIALIZER_LEN &&
      memcmp(parser.previous.start, INITIALIZER, INITIALIZER_LEN) == 0) {
...
static void super_(bool canAssign) {
  if (currentClass == NULL) {
    error("Cannot use 'super' outside of a class method.");
...
static void key(bool canAssign) {
  expression();
...
static void unary(bool canAssign) {
  ...
  parsePrecedence(PREC_UNARY); // operand
...
ParseRule rules[] = {
  ...
  { NULL,     key,     PREC_CALL },       // TOKEN_LEFT_BRACKET
  ...
  { NULL,     NULL,    PREC_EQUALITY },   // TOKEN_EQUAL
